        ...
        throw std::runtime_error(
            RPCHelpMan{"dxGetOrderHistory",
                "\nReturns the order history over a specified time interval.\n",
                {
                    ...
                    {"order_ids", RPCArg::Type::BOOL, "false", "If true, returns the GUIDs of all filled orders in each slice. If false, GUIDs are omitted."},
    ...
    if (request.fHelp)
        throw std::runtime_error(
            RPCHelpMan{"dxGetOrder",
                "\nReturns order info by order UUID.\n",
                {
                    {"id", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The order UUID."},
                },
                RPCResult{
                "\n"
                },
...
UniValue dxMakeOrder(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            RPCHelpMan{"dxMakeOrder",
                ...
                "\nNote:\n"
                "XBridge will first attempt use funds from the specified maker address. \n"
    ...
    if (params.size() < 7) {
        throw runtime_error("dxMakeOrder (maker) (maker size) (maker address) (taker) (taker size)\n"
                            "(taker address) (type) (dryrun, default=false)[optional]\n"
                            "Create a new order. You can only create orders for markets with tokens\n"
                            "supported by your node. There are no fees to make orders. [dryrun] will\n"
                            "validate the order without submitting the order to the network.");
    }
...
UniValue dxTakeOrder(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            RPCHelpMan{"dxTakeOrder",
                ...
                "[dryrun] will evaluate input without accepting the order.\n"
                "\nNote:\n"
                "XBridge will first attempt use funds from the specified from_address. \n"
                "If this address does not have sufficient funds to cover the order, then\n"
                "it will pull funds from other addresses in the wallet. Change is\n"
                "deposited to the address with the largest input used.\n",
                {
                    {"id", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The UUID of the order being filled."},
    ...
    {
        return uret(xbridge::makeError(xbridge::INVALID_PARAMETERS, __FUNCTION__,
                               "(id) (from_address) (to_address) (dryrun, default=false)[optional]"));
    }
    ...
    if(request.fHelp)
        throw std::runtime_error(
            RPCHelpMan{"dxCancelOrder",
                ...
                {
                    {"id", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The UUID of the order to cancel."},
                },
                RPCResult{
                "\n"
                },
    ...
    if (request.fHelp)
        throw std::runtime_error(
            RPCHelpMan{"dxGetOrderBook",
                ...
                "Detail 4 - Returns the best bid and ask with the order GUIDs.\n",
                {
    ...
    if (request.fHelp)
        throw std::runtime_error(
            RPCHelpMan{"dxGetLockedUtxos",
                ...
                {
                    {"id", RPCArg::Type::STR_HEX, RPCArg::Optional::OMITTED, "The order UUID. If omitted, a list of UTXOs used in all orders will be returned."},
                },
                RPCResult{
                "\n"
                },
